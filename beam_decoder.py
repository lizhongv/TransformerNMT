...
class Beam:
    ...
    def __init__(self, size, pad, bos, eos):
        ...
        self.scores = torch.zeros((size,), dtype=torch.float).cuda()
        self.all_scores = []
        ...
        self.next_ys = [torch.full((size,), self.PAD, dtype=torch.long).cuda()]
...
def beam_search(model, src, src_mask, max_len, pad, bos, eos, beam_size):
    ...
    with torch.no_grad():
        ...
        # -- Prepare beams
        inst_dec_beams = [Beam(beam_size, pad, bos, eos) for _ in range(batch_size)]
